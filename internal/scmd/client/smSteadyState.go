...
func (cmd *Command) dialTunRaw(dest string) (conn io.ReadWriteCloser, code byte) {
	...
	var myss *niaucchi3.Context
	myss = cmd.currTunn
	if myss == nil {
		log.Println("0x03 due to currTunn being null")
		code = 0x03
		return
	}
	conn, err = myss.Tunnel()
	if err != nil {
		myss.Tomb().Kill(io.ErrClosedPipe)
		log.Println("0x03 due to myss is BAD!!")
		code = 0x03
		return
	}
	...
	tmr := time.AfterFunc(time.Second*15, func() {
		myss.Tomb().Kill(niaucchi3.ErrTimeout)
	})
	b := make([]byte, 1)
